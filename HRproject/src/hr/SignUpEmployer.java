...
public class SignUpEmployer extends JFrame {
	/**
	 * 
	 */
	...
	private JTextField tfFirstname,tfLastname,tfEmail,tfUsername,tfPhone;
	private JPasswordField tfPassword,tfRepassword;
	private JLabel lbShow,lbLine,lbFirstname,lbLastname,lbEmail,lbUsername,lbPassword,lbRepassword,lbPhone;
	...
	public void initControls(){
		...
		pan.setLayout(null);
		lbShow=new JLabel("Sign Up new Employer");
		...
		pan.add(lbLine);
		lbFirstname=new JLabel("Company");
		...
		pan.add(tfFirstname);
		lbLastname=new JLabel("Contact Person");
		...
		lbPhone.setBounds(150,220,150,20);
		pan.add(lbPhone);
		tfPhone=new JTextField();
		tfPhone.setBounds(300,220,300,30);
		tfPhone.setBackground(new Color(204,229,255));
		pan.add(tfPhone);
		lbUsername=new JLabel("Username");
		lbUsername.setBounds(150,260,150,20);
		pan.add(lbUsername);
		tfUsername=new JTextField();
		tfUsername.setBounds(300,260,300,30);
		tfUsername.setBackground(new Color(204,229,255));
		pan.add(tfUsername);
		lbPassword=new JLabel("Password");
		lbPassword.setBounds(150,300,150,20);
		pan.add(lbPassword);
		tfPassword=new JPasswordField();
		tfPassword.setBounds(300,300,300,30);
		tfPassword.setBackground(new Color(204,229,255));
		pan.add(tfPassword);
		lbRepassword=new JLabel("Re-Password");
		lbRepassword.setBounds(150,340,150,20);
		pan.add(lbRepassword);
		tfRepassword=new JPasswordField();
		tfRepassword.setBounds(300,340,300,30);
		...
		chOk.setBounds(200,380,300,20);
		chOk.setBackground(Color.WHITE);
		pan.add(chOk);
		btnRegister=new JButton("Register");
		btnRegister.setBounds(200,420,150,30);
		btnRegister.setBackground(new Color(204,229,255));
		btnRegister.setEnabled(false);
		pan.add(btnRegister);
		btnCancel=new JButton("Cancel");
		btnCancel.setBounds(370,420,150,30);
...
chOk.addActionListener(new ActionListener(){
			...
			public void actionPerformed(ActionEvent e) {
				if(chOk.isSelected()){
					...
					if(tfFirstname.getText().isEmpty()){
						JOptionPane.showMessageDialog(null,"Please Input Company");
						//btnRegister.setEnabled(false);
						chOk.setSelected(false);
					}
					else if(tfLastname.getText().isEmpty()){
						JOptionPane.showMessageDialog(null,"Please Input Contact Person");
						//btnRegister.setEnabled(false);
						chOk.setSelected(false);
		...
		public void actionPerformed(ActionEvent arg0) {
			try{
				...
				String sql1="SELECT * FROM employer WHERE username='"+tfUsername.getText()+"' ";
				String sql2="INSERT INTO employer (company,contactPerson,email,phone,username,password) VALUES(?,?,?,?,?,?)";
				...
				if(res.next()){
					...
					resPre.setString(4, tfPhone.getText());
					resPre.setString(5, tfUsername.getText());
					resPre.setString(6, tfPassword.getText());
					resPre.executeUpdate();
					JOptionPane.showMessageDialog(null, "Register Company : "+tfFirstname.getText()+" Successfull.");
				}
				res.close();
				st.close();
				conn.close();
